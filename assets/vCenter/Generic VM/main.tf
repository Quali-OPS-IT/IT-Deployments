...
resource "vsphere_virtual_machine" "vm" {
  ...
  # Either keep template disk0 size OR resize it if disk_gb > 0
  disk {
    label       = "disk0"
    unit_number = 0

    size = local.resize_disk ? var.disk_gb : data.vsphere_virtual_machine.template.disks[0].size
